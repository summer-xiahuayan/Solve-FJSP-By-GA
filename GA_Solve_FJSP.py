...
class GA:
    ...
    def Select(self,Fit_value):
        ...
        for i in range(len(Fit_value)):
            ...
            Fit.append(fit)
        Fit=np.array(Fit)
        idx = np.random.choice(np.arange(len(Fit_value)), size=self.Pop_size, replace=True,
                               p=(Fit) / (Fit.sum()))
        return idx
    ...
    def main(self,Processing_time,J,M_num,J_num,O_num):
        ...
        for i in tqdm(range(self.Max_Itertions)):
            ...
            decode_stage=[0 if Fit[0][i]<=Fit[1][i] else 1 for i in range(len(Fit[0]))]
            best_index=min(Fit).index(min(min(Fit)))
            best_decode_stage=decode_stage[best_index]
            Best = C[best_index]
            best_fitness = min(min(Fit))
            if best_fitness < Optimal_fit:
                Optimal_fit = best_fitness
                Optimal_CHS = Best
                Best_fit.append(Optimal_fit)
                print('best_fitness:',best_fitness)
                print('decode_stage:',best_decode_stage)
                d = Decode(J, Processing_time, M_num)
                #Fit.append(d.Decode_1(Optimal_CHS, Len_Chromo))
                if best_decode_stage==0:
